...
const mostarTabela = () => {
    ...
    if(localStorage.getItem("jogoPalavra")){
        ...
        for(let i = 0; i < palavras.length; i++){
            const linha = tbPalavras.insertRow(-1) // add uma linha na tabela
...
ckMostrar.addEventListener("change", () =>{
    ckMostrar.checked ? mostarTabela() : window.location.reload()
})
...
tbPalavras.addEventListener("click", (e) => {
    if(e.target.classList.contains("exclui")) {
        ...
        if(confirm(`Confirma Exclusão da Palavra: "${palavra}"?`)) {
            ...
            localStorage.setItem("jogoPalavra", palavra.join(";"))
